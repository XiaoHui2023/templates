...
| 成员 / 配置 | 说明 |
| --- | --- |
| `sqr` | **kit_interface** 句柄；**callback** 注册、**trees** 访问、配置与 **interface** 行为便捷方法均在此句柄上 |
...
派生 **sequencer**；承载 **configure_tree** 等配置便捷方法。**agent.sqr** 类型为 **kit_interface**，本类为其基类。
...
派生 **kit_sequencer**；**agent.sqr** 的实际类型。在 **interface** 上启动 **sequence/behavior** 的便捷入口。
...
每种行为独占 **`sequence/behavior/<行为名>/`**，含 **req**、**rsp** 两个 **uvm_sequence_item** 与 **behavior** 序列。**behavior** 为 **`uvm_sequence#(REQ,RSP)`**，由 **kit_interface** 填入 **req** 后 **start**，再取 **rsp**。全仓约定见用户根 **`systemverilog-uvm-sequence-behavior`**。
