...
class your_prefix_my_cb extends your_prefix_callback;
  ...
  virtual function void on_configure_settings(your_prefix_settings settings);
...
## `on_configure_settings`

按 **settings** 字段完成系统配置，如写寄存器或时钟相关设置；配置完成后对已绑定的各 **tree** 做时钟树检查。对 **`sqr.configure_settings`** 或 **`sqr.configure_tree`** / **`configure_all_trees`** 时调用。

| 方向 | 类型 | 参数名 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| input | settings | settings | 无 | 当前要应用的设置实例，通常来自 **`tree.settings`** |
