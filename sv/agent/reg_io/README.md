...
模板配置项（`class_prefix`、`input_port_name`、`output_port_name`）的含义以同目录 `models.py` 里各字段的 `Field(..., description=...)` 为准。
...
| port | 方向 | 类型 | 说明 |
| --- | --- | --- | --- |
| `i_ap` | input | `uvm_analysis_port #(uvm_tlm_generic_payload)` | 外部写事务输入；解析并写回寄存器模型，`dump_file` 非空时记入待写出队列 |
| `o_ap` | output | `uvm_analysis_port #(uvm_tlm_generic_payload)` | CSV 初始化或 `load_csv` 产生的写事务输出 |
