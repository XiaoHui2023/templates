...
class Models(BaseModel):
    ...
    | 默认名 `o_<名>_ap`（各输出 `out_port_name`） | output | `uvm_analysis_port #(对应输出类型)` | 转换后写出 |
    ...
    每个输出的函数名由该项的 `hook_name` 决定；若未配置，一般为前缀 `to_` 加上该条目的 `name` 字段。签名与模板声明一致：第一个参数为输入事务（参数名由 `name_input_data` 配置），第二个为输出，类型为该条的 `class_name`；若该条 `is_queue` 为真，输出为对应类型的动态数组。
