...
flash 相关快捷入口可以输入单次传输配置。默认值由 Python model 生成；`dma_test.speed_multiplier` 默认使用支持的最高倍速。
...
仅在 `internal_dma` 或 `external_dma` 开启时存在。每次只测试 `speed_multiplier` 指定的一种 enhanced 速度，默认使用 `max_speed_multiplier`；入口强制 DMA，不暴露 `use_dma`。内部 DMA 额外接受 `axi_addr`，`awlen/arlen` 固定为 15。外部 DMA mover 由 callback 注入。
