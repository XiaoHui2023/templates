...
- PIO `EEPROM_READ` derives leading receive bytes from `dummy_cycles * io_lanes / 8` and removes them before scoreboard comparison.
- DMA read uses `RX_ONLY`, so `rx_skip_bytes` is 0.
