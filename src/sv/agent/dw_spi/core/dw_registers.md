...
当前内置 read opcode 的 wait/dummy SCLK 数为：`READ1X 03h=0`、`FASTREAD1X 0Bh=8`、`READ2X BBh=4`、`READ4X EBh=6`。后三者在 enhanced `EEPROM_READ` 路径写入 `WAIT_CYCLES`。`READ2X` 配置 `rx_skip_bytes=addr_bytes`，`READ4X` 配置 `rx_skip_bytes=3`；接收丢弃量不属于 `WAIT_CYCLES`。
