...
当前内置 NOR-like read packet 使用 `03h=8`、`0Bh=16`、`BBh=12`、`EBh=10` 个 SCLK cycle。标准 `03h` 路径把 8 cycles 写成一个 DR dummy byte；enhanced 路径把对应值直接写入 `SPI_CTRLR0.WAIT_CYCLES`。
