...
Python `internal_dma` 和 `external_dma` 互斥。两者都为 false 时，不生成 DMA 字段和 DMA 寄存器配置。内部 DMA operation req 的 `awlen/arlen` 是四位单笔 burst 长度字段，由 adapter 按控制项和 payload 的 32-bit AXI beat 数推导并限制为 0-15，不作为 transfer configuration 或 kit 输入。总量超过 16 beat 时，内部 DMA 在同一次 SPI transaction 内发起多笔 AXI burst。
...
接收整形和前置流程也由指令包声明：`rx_skip_bytes` 表示实际接收后需要丢弃的前导 byte 数，`requires_qe` 表示执行该指令前是否运行 QE 配置流程。flow 只消费这些字段，不根据速度或 opcode 再次推导。1x 使用 standard `READ1X 0x03`；`READ2X 0xBB` 配置 `rx_skip_bytes=1`；`READ4X 0xEB` 配置 `dummy_cycles=6`、`rx_skip_bytes=3`、`requires_qe=1`。
...
| `ssi_en` / `ser` / `baudr` | 配置 `SSIENR`、`SER`、`BAUDR`；`baudr` 是最终 `SCKDV` 编码，DMA 时为逻辑波形分频值的一半 |
