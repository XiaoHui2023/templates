...
指令包用 `requires_qe` 表示执行指令前是否运行 QE 配置流程。最终 `transfer_req` 根据 `transfer_mode`、`dummy_cycles` 和 `io_lanes` 约束 `rx_skip_bytes`；PIO `EEPROM_READ` 计算前导丢弃量，DMA `RX_ONLY` 为 0。1x 使用 standard `READ1X 0x03`；`READ4X 0xEB` 配置 `dummy_cycles=6` 和 `requires_qe=1`。
