...
![待测节点视角：强调穿过待测节点的支路](images/test_route.drawio.svg)

为避免多路选择切换后频率不变、无法区分支路，基线须令各 **div**、**dto** 分频比为 1，各 **PLL** 的 **fix_mul** 从 2 起依次递增且与晶振频率不同。

### 具体方法

1. **基线固定**：全部 **gate** 的 **fix_open** 为 1；全部 **div**、**dto** 的 **fix_ratio** 为 1；各 **PLL** 的 **fix_mul** 从 2 起递增；各 **mux** 的 **fix_sel** 指向可达任一 **clk** 的前级支路。对 **tree** 执行一次 **randomize**，再 **config_reg** 写整树寄存器。
...
4. **结构探测**：对每个已绑寄存器的 **gate**、**mux**、**div**、**dto** 节点，分别做**上游**与**下游**探测：
   ...
   4. 该方向完成后从基线快照恢复控制量并写回寄存器，再测下一节点。
...
| 错误点 | 使用者可见现象 | 覆盖方式 |
| --- | --- | --- |
| 基线频率不符 | **check_freq** 在步骤 2 失败 | **fatal**，不进入结构探测 |
...
| --- | --- |
| **config_reg** | 基线整树写寄存器；探测时只写变化的节点 |
...
在树状拓扑与基线频率可区分的前提下：步骤 2、3 保证度量前提成立；步骤 4 对每个节点的上游、下游分别用可控寄存器扰动线节点，若模型前后级与 RTL 一致，则 **check_freq** 应与当前 **open**、**sel**、**ratio** 传播后的 **frequence** 一致；局部均通过则各节点前后级与配置图一致。
