...
4. **结构探测**：**required_clk_nodes** 非空时，未列入的 **clk** 设 **unfix** 为 1，所列 **clk** 保持 **unfix** 为 0；**required_clk_nodes** 为空时全部 **clk** **unfix** 为 0。对每个已绑寄存器的 **gate**、**mux**、**div**、**dto** 节点，分别做**上游**与**下游**探测：
