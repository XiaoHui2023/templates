...
- DMA 模式使用 IDMAC 描述符链表，不复用 MSHC ADMA2 描述符模型；`CTRL_R.USE_INTERNAL_DMAC` 选择 internal DMAC，`DBADDR_R` 写 `idmac_descriptor.descriptor_addr`，DES2 写数据 buffer 地址，`BMOD_R.SWR` 复位 IDMAC，`BMOD_R.DE` 开启 IDMAC，`PLDMND_R` 写 `32'h1` 触发。
...
- 生成 DMA 时提供 `dma_test()`；它复用 `rw_test_seq`，固定 `dma_enable == 1`，默认单块写后读。MSHC 保留 ADMA2 默认，mobile_storage 走 IDMAC。
