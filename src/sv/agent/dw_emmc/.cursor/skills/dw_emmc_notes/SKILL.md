...
- DMA 模式使用 IDMAC 描述符链表，不复用 MSHC ADMA2 描述符模型；`DBADDR_R` 写 `idmac_descriptor.descriptor_addr`，DES2 写数据 buffer 地址，`BMOD_R.SWR` 复位 IDMAC，`BMOD_R.DE` 开启 IDMAC，`PLDMND_R` 写 `32'h1` 触发。
