...
`mobile_storage` 只走 IDMAC 描述符链表模型：`DBADDR_R` 写 `adma_des.cmd_addr`，真实数据 buffer 地址写在描述符 `real_addr` 中，写完 `DBADDR_R` 后向 `PLDMND_R` 写 `32'h1` 触发 DMA；`0x84` 是 `PLDMND_R` 地址。
