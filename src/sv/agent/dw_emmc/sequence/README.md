...
- mobile_storage 普通命令写 `CMD_R.START_CMD` 前清 CMD complete 旧状态并配置 `INTMASK_R`。
- mobile_storage 的普通中断由 `wait_interrupt` 清除，`check_error` 不能写回普通中断位；`wait_interrupt` 先读 `MINTSTS_R` 消费已置位状态，不在入口全清 `RINTSTS_R`。
