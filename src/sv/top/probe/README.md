...
频率检查的超时上限由 `MIN_FREQ_HZ` 和 `STABLE_CYCLES` 推导，覆盖首个有效上升沿和连续测量周期。inactive 检查只观察一个最低频率周期。总入口另有同样由这两个参数推导的总超时保护；任一子检查未按时返回时，`pass` 保持为 `0` 并报告 `probe: timeout`。不需要额外配置超时参数。
