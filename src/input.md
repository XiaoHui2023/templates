...
渲染前，工具先把配置文件读成 dict，再用 `Models(**data)` 实例化。下列能力由 **configlib** 提供；扩展名常见 `.yaml`、`.yml`、`.json`、`.json5`、`.toml`、`.csv` 等，由加载器按后缀选择。
...
部分载入工具支持：首条非空行形如 `#a.yaml`，把一个物理文件拆成多段虚拟文件；**第一段**为入口，段内仍可用 `!include` 引用同一文件内其它段名。
